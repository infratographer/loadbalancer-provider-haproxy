...
echo "Creating NATS account for loadbalancer-provider-haproxy"
nsc add account -n PROV -K ${OPERATOR_SIGNING_KEY_ID}
nsc edit account PROV --sk generate --js-mem-storage -1 --js-disk-storage -1 --js-streams -1 --js-consumer -1
nsc describe account PROV

export ACCOUNTS_SIGNING_KEY_ID=`nsc describe account PROV -J | jq -r '.nats.signing_keys | first'`

echo "Creating NATS user for loadbalancer-provider-haproxy"
